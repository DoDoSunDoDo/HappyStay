...
public class ImageStorageService {
    ...
    public String save(MultipartFile file) throws GCSUploadException {
        ...
        try {
            credentials = GoogleCredentials.fromStream(getClass().getClassLoader().getResourceAsStream("staybooking-key.json"));
        } catch (IOException e) {
            throw new GCSUploadException("Failed to load GCP credentials");
        }
        Storage storage = StorageOptions.newBuilder().setCredentials(credentials).build().getService();

        String filename = UUID.randomUUID().toString();//随机生成一个名字
